...
class RoutesConsistencyTest < ActionDispatch::IntegrationTest
  ...
  test "routes and employee listings come from persisted records" do
    ...
    machine = Machine.create!(id: "M-200", name: "Lobby Machine", lat: 42.0, lng: -71.0, location: "HQ Lobby")
    ...
    assert_response :success
    assert_equal ["emp_1"], json_response.map { |row| row["employeeId"] }
    ...
    assert_response :success
    assert_equal ["M-200"], json_response.fetch("stops").map { |row| row["machineId"] }
  end
end
